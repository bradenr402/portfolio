...
Instead of chaining multiple {% sidenote-ref label="sub-variants" %}`gsub`{% /sidenote-ref %} calls, you can pass a hash as the replacement. Each match is replaced by its corresponding value (or removed if no key exists).
